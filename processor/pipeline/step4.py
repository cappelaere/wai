...
Examples:
  ...
  python code/step4.py --markdown-only
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="Limit to top N applicants by total score (default: 20, use --limit N for top N applicants; set to 0 to process all)"
    )
        ...
        if top_applicants:
                        ...
                        with open(csv_file, 'r', encoding='utf-8') as f:
                            ...
                            for row in top_rows:
                                ...
                                if key in applications_dict:
                                    ...
                                    wai_number = (
                                        row.get('wai_application_number', '').strip()
                                        or row.get('wai_membership_number', '').strip()
                                        or "N/A"
                                    )
                                    email = row.get('email', '').strip() or "N/A"
            ...
            if selected_applicants_log:
                ...
                for i, app in enumerate(selected_applicants_log, 1):
                    logging.info(
                        f"  #{i}: {app['name']} (WAI: {app['wai_number']}, "
                        f"Email: {app['email']}, Score: {app['total_score']}, "
                        f"Folder: {app['scholarship']}/{app['application_folder']})",
                        extra=extra
                    )
        else:
            # When --limit is used, we require top applicants to be determined from CSV.
            # Do NOT fall back to processing all applications, to avoid mixing in non-top applicants.
            print("ERROR: No top applicants could be selected from applicant_scores_summary.csv.")
            print("       When using --limit, Step 4 requires the CSV generated by step3.py")
            print("       (applicant_scores_summary.csv sorted by total_score).")
            if args.scholarship_folder:
                print(f"       Checked scholarship folder: {args.scholarship_folder}")
            else:
                print(f"       Checked all scholarship folders under: {output_dir}")
            sys.exit(1)
    else:
        if verbose:
